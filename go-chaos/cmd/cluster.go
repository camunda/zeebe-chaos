...
func scaleCluster(flags *Flags) error {
	if flags.partitionCount <= 0 && flags.brokers <= 0 && flags.replicationFactor <= 0 {
		internal.LogInfo("Either partitionCount, replicationFactor or brokers must be set for scale command")
	}
