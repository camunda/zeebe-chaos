...
Without Optimize, the cluster completes 22% more process instances per second and reaches 91.4% completion, up from 71.5%. The additional ES write pressure from Optimize's analytics indices causes the exporter to fall behind, which, in turn, drives up backpressure on the brokers.
...
#### Max workload: Latency

![Latency comparison — max scenario](max-latency.png)

| Metric | Base Max | No-Opt Max | Delta |
|---|---|---|---|
| Write-To-Processing latency | 3.25 ms | 6.79 ms | Factor ~2.1 |
...
![Latency comparison — realistic scenario](real-latency.png)

| Metric | Base Realistic | No-Opt Realistic | Delta |
|---|---|---|---|
| Write-To-Processing latency | 26.8 ms | 20.5 ms | ~6.3 ms |
...
The difference in ES disk usage is even more significant under the realistic workload. The cluster with Optimize enabled is using 44.3% of the ES disk capacity, while the cluster without Optimize is using only 12.3%. This means that Optimize consumes an additional 32% of the ES disk capacity under realistic load, a substantial increase that highlights the significant storage overhead it introduces to Elasticsearch.

To speak in total numbers instead of percentages, 32% of 500 GiB is 160 GiB. This is additional disk space consumed by Optimize, something which should be considered during cluster sizing and capacity planning for production environments.
...
- **Optimize's ES CPU overhead is disproportionately large at low throughput.** At a realistic workload with zero backpressure, Optimize consumes 3.4x more ES CPU than the base workload requires. This overhead is not throughput-proportional — it is likely related to workload factors such as variable usage and process structure (multi-instance, call activity, etc.).
- **Optimize dominates Elasticsearch disk usage.** After two days, the realistic Optimize cluster used ~4x more ES disk than the no-Optimize cluster. For customers running Optimize at production scale, ES storage sizing must take this into account.
...
- **Investigate new architecture.** The team is exploring new ideas for an improved Optimize architecture that should substantially reduce resource overhead. The findings here provide a concrete baseline to measure that improvement against.
