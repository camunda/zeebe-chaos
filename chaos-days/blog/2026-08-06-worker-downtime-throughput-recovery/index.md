...
**TL;DR:** We took a job worker down for 80 minutes. Throughput recovered within seconds of it returning, but the backlog behind it grew for another two hours, because the step we removed fans one process instance into 50 downstream jobs. Adding capacity and replicas moved the throughput number and not the backlog, and that number was never progress: a job counts as handled when its handler returns, even after the broker has timed it out and thrown the completion away. Two layers sit underneath. Job push hands new jobs straight to a worker, so they never enter the queue the backlog lives in, leaving polling to drain it while competing with push for the same capacity. And the client's poll path has three defects of its own: two explain what we measured, the third we could only reproduce in a test. Filed as [camunda/camunda#59631](https://github.com/camunda/camunda/issues/59631) for the engine side and [camunda/camunda#59635](https://github.com/camunda/camunda/issues/59635) for the client.
...
We were interested in the details and traced the engine, gateway and client code, since "the backlog isn't shrinking while throughput looks fine" needed an explanation. We started from the metrics and a hypothesis that job push was being prioritised, then went looking for where that prioritisation actually lives.
